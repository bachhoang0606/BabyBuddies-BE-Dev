...
const searchStaff = async (filters) => {
    try {
        ...
        async function calculateMatchingScore(employee, filters) {
            ...
            const { rating, salary, userLanguage, careExp, cookExp, address } =
                filters;
            ...
            if (userLanguage !== undefined) {
                const userLanguageName = await Language.find({
                    _id: { $in: employee.userLanguage },
                }).distinct('name');
                const isMatchingLanguage =
                    userLanguageName.includes(userLanguage);
        ...
        async function searchEmployees(filters) {
            ...
            for (let i = 0; i < employees.length; i++) {
                ...
                const { matchingScore, numFailedConditions } =
                    matchingResults[i];
